...
const ProgressStepper = ({ currentStep, onStepClick }) => {
  return React.createElement('div', { className: 'w-full p-2' },
    React.createElement('div', { className: 'flex items-center justify-center relative' },
      React.createElement('div', { className: 'absolute left-0 right-0 top-6 md:top-7 -translate-y-1/2 h-1 w-[calc(100%-48px)] sm:w-[calc(100%-56px)] mx-auto bg-zinc-800' },
      ...
      React.createElement('div', { className: 'flex justify-between w-full max-w-xs sm:max-w-md' },
        steps.map((step) => {
          const status =
          ...
          return React.createElement('div', { key: step.id, className: 'relative z-10 flex flex-col items-center' },
            React.createElement(StepWrapper, {
              ...
              status === 'active' && React.createElement(motion.div, {
              ...
              status === 'completed' ? React.createElement(Check, { className: 'w-7 h-7 md:w-8 md:h-8' }) : React.createElement(Icon, { className: 'w-6 h-6 md:w-7 md:h-7' })
            ),
            React.createElement('p', {
              className: `mt-2 md:mt-3 text-xs sm:text-sm md:text-base font-semibold text-center transition-colors duration-300
